...
import net.thevpc.common.prs.plugin.BlendedFile;
...
import net.thevpc.dbclient.api.pluginmanager.DBCPluginManagerImpl;
import net.thevpc.dbclient.api.pluginmanager.DBCPlugin;
import net.thevpc.dbclient.api.pluginmanager.DBCPluginManager;
...
public final class DBCApplicationImpl implements DBCApplication {
    ...
    public void start(Properties applicationProperties, ProgressMonitor monitor) {
        ...
        if (Boolean.getBoolean("ResetVarFolder")) {
            PRSPrivateIOUtils.deleteFolderTree(getVarDir(), null);
        }
...
//        }
        ...
        if (!PRSPrivateIOUtils.isFolderWritable(wd)) {
            MessageDialogManager.ReturnType r = getView().getDialogManager().showMessage(null,
                    ...
                    null,
                    null,
                    new DBCInitMessageDiscardContext("[" + PRSPrivateIOUtils.getFilePath(wd) + "].change_workdir_to_hwd"));
