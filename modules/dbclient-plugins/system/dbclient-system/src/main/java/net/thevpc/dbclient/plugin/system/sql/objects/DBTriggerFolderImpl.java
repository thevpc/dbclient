...
import net.thevpc.dbclient.api.sql.objects.DBTrigger;
import net.thevpc.dbclient.api.sql.objects.DBTriggerFolder;
 ...
 */
public class DBTriggerFolderImpl extends DBObjectFolderImpl implements DBTriggerFolder {

    public void init(DBCConnection session, String catalogName, String schemaName, String tableName) throws SQLException {
        super.init(session, "Tree.TableTriggers", catalogName, schemaName, tableName, "TableTriggers", false);
//        setSystemObject(session.isSystemType(name));
    ...
    protected void loadChildren() throws SQLException {
        ResultSet rs = getConnection().getTriggers(
                getCatalogName(),
                getSchemaName(),
                getParentName(),
                "%",
        ...
        if (rs != null) {
            while (rs.next()) {
                DBTrigger item = connection.getFactory().newInstance(DBTrigger.class);
                item.init(getConnection(), getCatalogName(), getSchemaName(), getParentName(), rs.getString(4));
                addTrigger(item);
            }
            rs.close();
        }
