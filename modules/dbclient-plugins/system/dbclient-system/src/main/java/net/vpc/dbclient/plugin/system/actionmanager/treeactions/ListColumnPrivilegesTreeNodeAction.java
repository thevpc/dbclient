...
public class ListColumnPrivilegesTreeNodeAction extends DBCTreeNodeAction {

    public ListColumnPrivilegesTreeNodeAction() {
        super("Action.ListColumnPrivilegesTreeNodeAction");
        addLocationPath(DBCActionLocation.POPUP, "/List");
    }

    public boolean shouldEnable(DBObject activeNode, DBObject[] selectedNodes) {
        return (activeNode != null && 
                SystemSQLUtils.isTableDown(activeNode)
        );
    }
    ...
    public void actionPerformedImpl(ActionEvent e) throws Throwable {
        ...
        table.getModel().setTitle("TableColumn");
        ...
        table.setRefreshAction(new DBCResultTableAction("Refresh", getPluginSession()) {
            ...
            public void actionPerformedImpl(ActionEvent e) throws Throwable {
                ...
                model.setTitle("TableColumn");
                model.displayQuery(getSession().getConnection().getMetaData().getColumnPrivileges(
                        cp.getCatalogName(),
                        cp.getSchemaName(),
                        cp.getTableName(),
                        cp.getColumnName()), null);
            }
        });
        table.refresh();
