...
public class ListTableIndexInfosTreeNodeAction extends DBCTreeNodeAction {

    public ListTableIndexInfosTreeNodeAction() {
        super("Action.ListTableIndexInfosTreeNodeAction");
        addLocationPath(DBCActionLocation.POPUP, "/List");
    }

    public boolean shouldEnable(DBObject activeNode, DBObject[] selectedNodes) {
        return (activeNode!=null) && 
//                (SystemSQLUtils.isTableIndexFolderPath(activeNode)
                (activeNode instanceof DBCTable)
                ;
    }
    ...
    public void actionPerformedImpl(ActionEvent e) throws Throwable {
        ...
        table.setRefreshAction(new DBCResultTableAction("Refresh", getPluginSession()) {
            ...
            public void actionPerformedImpl(ActionEvent e) throws Throwable {
                ...
                model.displayQuery(getSession().getConnection().getMetaData().getIndexInfo(
                        cp.getCatalogName(),
                        cp.getSchemaName(),
                        cp.getTableName(),
                        false,
