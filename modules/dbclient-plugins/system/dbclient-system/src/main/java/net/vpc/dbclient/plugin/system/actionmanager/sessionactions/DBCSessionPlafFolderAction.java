 ...
 */
public class DBCSessionPlafFolderAction extends DBCSessionFolderAction {

    public DBCSessionPlafFolderAction() {
    ...
    public Component addToComponent(Component component) {
        ...
        if (getSession().getApplication().getView().getLookAndFeelManager().isSupported()) {
            plafMenu = new JPlafMenu(null) {
                ...
                public void applyPlaf(PlafItem item) throws IllegalAccessException, UnsupportedLookAndFeelException, InstantiationException, ClassNotFoundException {
                    getSession().getView().setPlaf(item);
                }
            };
        }
        if (plafMenu != null) {
            if (component instanceof JPopupMenu) {
                ...
                getSession().getLogger(DBCSessionPlafFolderAction.class.getName()).log(Level.SEVERE, "(id={0} for component type ({1}) unsupported", new Object[]{getKey(), component.getClass().getName()});
            }
        }
        return null;
    }
