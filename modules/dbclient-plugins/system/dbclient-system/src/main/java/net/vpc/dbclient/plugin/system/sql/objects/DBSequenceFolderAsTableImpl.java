 ...
 */
public class DBSequenceFolderAsTableImpl extends DBObjectFolderImpl implements DBSequenceFolderAsTable {
    ...
    protected void loadChildren() throws SQLException {
        ResultSet rs = getConnection().getMetaData().getTables(
                ...
                new String[]{typeName});
        while (rs.next()) {
            DBSequenceAsTable item = getConnection().getFactory().newInstance(DBSequenceAsTable.class);
            item.init(getConnection(), rs);
            DBSequenceAsTable tn = item;
