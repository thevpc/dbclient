 ...
 */
public class KeywordCollectionConvertChecker extends CollectionWordTokenResolver {
    public KeywordCollectionConvertChecker(Map<String, Integer> idMap, Collection<String> all) {
        super(SQLTokenGroup.KEYWORD, idMap, all);
    }

    public KeywordCollectionConvertChecker(Map<String, Integer> idMap, String... all) {
        super(SQLTokenGroup.KEYWORD, idMap, all);
    }

    @Override
    public TokenRichType resolve(String word, int supposedType, SQLParser parser) throws IOException {
        if (supposedType == SQLTokenGroup.VAR) {
