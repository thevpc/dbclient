...
//            e.printStackTrace();
    ...
    protected String getResourceString(String urlPath) throws IOException {
        ...
        return IOUtils.loadStreamAsString(resource);
    }
    ...
    protected ResultSet getSQLResourceValueAsResultSet(String urlPath, Map<String, String> params) throws SQLException, IOException {
        ...
        while (again) {
            ...
            for (Map.Entry<String, String> s : params.entrySet()) {
                ...
                int i = sql.indexOf(k);
                if (i >= 0) {
                    sql = StringUtils.substring(sql, 0, i) + s.getValue() + StringUtils.substring(sql, i + k.length(), sql.length());
                    again = true;
                }
            }
        }
        ...
        while (again) {
            ...
            if (bestIndex >= 0) {
                sql = StringUtils.substring(sql, 0, bestIndex) + "?" + StringUtils.substring(sql, bestIndex + bestK.length(), sql.length());
                values.add(bestV);
                pos++;
                startIndex++;
                again = true;
