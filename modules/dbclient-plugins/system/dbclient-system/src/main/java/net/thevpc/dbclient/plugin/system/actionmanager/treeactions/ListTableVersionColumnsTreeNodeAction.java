...
public class ListTableVersionColumnsTreeNodeAction extends DBCTreeNodeAction {

    public ListTableVersionColumnsTreeNodeAction() {
        super("Action.ListTableVersionColumnsTreeNodeAction");
        addLocationPath(DBCActionLocation.POPUP, "/List");
    }

    public boolean shouldEnable(DBObject activeNode, DBObject[] selectedNodes) {
        return (activeNode != null && 
                //SystemSQLUtils.isTableColumnFolderPath(activeNode)
                activeNode instanceof DBTable
        );
    }
    ...
    public void actionPerformedImpl(ActionEvent e) throws Throwable {
        ...
        table.setRefreshAction(new DBCResultTableAction("Refresh", getPluginSession()) {
            ...
            public void actionPerformedImpl(ActionEvent e) throws Throwable {
                ...
                model.displayQuery(getSession().getConnection().getMetaData().getVersionColumns(
                        cp.getCatalogName(),
                        cp.getSchemaName(),
                        cp.getTableName()
                ), null);
