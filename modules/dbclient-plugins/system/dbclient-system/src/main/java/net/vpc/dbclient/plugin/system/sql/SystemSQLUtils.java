...
import net.vpc.dbclient.api.sql.objects.DBTableColumnFolder;
import net.vpc.dbclient.api.sql.objects.DBTableFolder;
import net.vpc.dbclient.api.sql.objects.DBTableIndex;
import net.vpc.dbclient.api.sql.objects.DBTableIndexFolder;
    ...
    public static String sqlToHtml(String sql) {
        ...
        while ((token = tok.readToken()) != null) {
            if (token.accept(SQLTokenGroup.OPEN_PAR)) {
                ...
                sb.append("<font color=blue>").append(StringUtils.textToHtml(token.getValue())).append("</font>");
            } else if (token.accept(SQLTokenGroup.STR)) {
                sb.append("<font color=green>").append(StringUtils.textToHtml(token.getValue())).append("</font>");
            } else if (token.accept(SQLTokenGroup.KEYWORD)) {
                sb.append("<font color=blue>").append(StringUtils.textToHtml(token.getValue())).append("</font>");
            } else if (token.accept(SQLTokenGroup.OPERATOR)) {
                sb.append("<font color=red>").append(StringUtils.textToHtml(token.getValue())).append("</font>");
            } else {
                sb.append(StringUtils.textToHtml(token.getValue()));
            }
        }
        return sb.toString();
    }
