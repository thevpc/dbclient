...
import net.vpc.dbclient.api.sql.objects.DBCluster;
import net.vpc.dbclient.api.sql.objects.DBClusterFolder;
 ...
 */
public class DBClusterFolderImpl extends DBObjectFolderImpl implements DBClusterFolder {

    public void init(DBCConnection session, String catalogName, String schemaName) {
        super.init(session, "Tree.AllClusters", catalogName, schemaName, null, "AllClusters", false);
    }
    ...
    protected void loadChildren() throws SQLException {
        ResultSet rs = getConnection().getClusters(
                getCatalogName(),
                getSchemaName(),
                "%", null);
        while (rs.next()) {
            DBCluster item = getConnection().getFactory().newInstance(DBCluster.class);
