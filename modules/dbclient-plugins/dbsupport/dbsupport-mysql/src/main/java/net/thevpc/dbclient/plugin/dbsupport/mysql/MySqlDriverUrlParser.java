...
package net.thevpc.dbclient.plugin.dbsupport.mysql;
 ...
 */
public class MySqlDriverUrlParser implements DBCDriverUrlParser {
    @Inject
    private DBCPlugin plugin;

    public MySqlDriverUrlParser() {
    }

    public int acceptDriver(String driver) {
        return driver.equals("org.gjt.mm.mysql.Driver") ? 10 : -1;
    }
    ...
    public Properties parse(String driver, String url) {
        ...
        if (strings.length >= 2 && strings[0].equals("jdbc") && strings[1].equals("mysql") && strings[2].startsWith("//")) {
