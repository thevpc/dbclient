...
package net.vpc.dbclient.plugin.dbsupport.postgresql;
 ...
 */
public class PostgreSqlDriverUrlParser implements DBCDriverUrlParser {
    @Inject
    private DBCPlugin plugin;

    public PostgreSqlDriverUrlParser() {
    }

    public int acceptDriver(String driver) {
        return driver.equals("org.postgresql.Driver") ? 10 : -1;
    }
    ...
    public Properties parse(String driver, String url) {
        ...
        if (strings.length >= 2 && strings[0].equals("jdbc") && strings[1].equals("postgresql") && strings[2].startsWith("//")) {
            String serverAndParams = strings[2].substring(2);
