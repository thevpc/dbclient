...
import org.vpc.neormf.jbgen.projects.JavaDAOTarget;
 ...
 */
class Target_java_dao_target extends NSettingPanel {
    ...
    public Target_java_dao_target(NeormfSettingsComponent neormfSettingsComponent) {
        super(JavaDAOTarget.NAME, "Java Project", neormfSettingsComponent);
    ...
    public void load(ConfigNode _target) {
        if (_target != null) {
            ...
            if (_dao_target != null) {
                ...
                log_api.setSelectedItem(getValidLogAPI(log));
    ...
    public static String getValidLogAPI(ConfigNode node){
        return getValidLogAPI((node!=null && node.isEnabled()) ? node.getValue():null);
    }
    
    public static String getValidLogAPI(String value){
        if(value!=null && JBGenProjectInfo.SUPPORTED_LOG_API_JAVA.contains(value)){
            return value;
        }
        return null;
    }
